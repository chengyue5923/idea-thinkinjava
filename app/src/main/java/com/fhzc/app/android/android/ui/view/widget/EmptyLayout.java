...
public class EmptyLayout {
    ...
    private String mLoadingMessage = "加载中,请稍候...";
    ...
    private int mEmptyImageResId = R.drawable.ic_empty;
    private int mEmptyImageViewId = R.id.view_empty_v2_iv;
    ...
    private void changeEmptyType() {
        ...
        if (!mViewsAdded) {
            ...
            if (mLoadingView != null)
                rl.addView(mLoadingView, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
            if (mErrorView != null)
                rl.addView(mErrorView, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
            mViewsAdded = true;
...
//					if (loadingAnimationView!=null && loadingAnimationView.getAnimation()!=null) loadingAnimationView.clearAnimation();
                    }
...
//					if (loadingAnimationView!=null && loadingAnimationView.getAnimation()!=null) loadingAnimationView.clearAnimation();
    ...
    private void refreshMessages() {
        if (mEmptyMessageViewId > 0 && mEmptyMessage != null) {
            TextView tv = ((TextView) mEmptyView.findViewById(mEmptyMessageViewId));
            tv.setText(mEmptyMessage);
            if (mEmptyImageResId != R.drawable.ic_empty) {
                /*Drawable drawable = mContext.getResources().getDrawable(mEmptyImageResId);
                drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
                tv.setCompoundDrawables(drawable,null,null,null);*/
                ImageView iv = (ImageView) mEmptyView.findViewById(mEmptyImageViewId);
                iv.setImageResource(mEmptyImageResId);
            }
        }
    ...
    private void setDefaultValues() {
        if (mEmptyView == null) {
            mEmptyView = (ViewGroup) mInflater.inflate(R.layout.view_empty_v2, null);
...
//
     ...
     */
    public EmptyLayout(Context context, View listView) {
        mContext = context;
        mInflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        mListView = listView;
        parent = (ViewGroup) mListView.getParent();
     ...
     * Shows error layout if the list is empty
     */
    public void showSuccess(boolean isEmpty) {
        this.mEmptyType = isEmpty?TYPE_EMPTY:TYPE_SUCCESS;
        changeEmptyType();
    }

    public void setmEmptyImageResId(int mEmptyImageResId) {
        this.mEmptyImageResId = mEmptyImageResId;
    }
}
