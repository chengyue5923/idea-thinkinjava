...
//    private Sensor mAccelerometerSensor;
    ...
    public MotionMonitorUtils(MotionMonitorUtilsDelegate delegate) {
        ...
        mSensorManager = (SensorManager) FuhuaApplication.getInstance().getApplicationContext().getSystemService(Context.SENSOR_SERVICE);
...
//                    if (maxValuex > valuex && valuex >= 0.0 && deltaTime < 4) 
                    if (valuex >= maxValuex - Float.MIN_VALUE && valuex <= maxValuex + Float.MIN_VALUE) {
                        AudioManager audioManager = (AudioManager) FuhuaApplication.getInstance().getApplicationContext().getSystemService(Context.AUDIO_SERVICE);
