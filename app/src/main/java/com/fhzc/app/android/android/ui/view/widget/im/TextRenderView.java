...
public class TextRenderView extends BaseMsgRenderView {
    ...
    public void render(MessageEntity messageEntity, ContactModel userEntity, final Context context) {
        ...
        if (CommonUtil.isShareText(content)) {
            final ShareModel model = CommonUtil.getShareModel(content);

            String show = model==null?content:model.getTitle();
            show=show+"\n 点击查看详情";
            SpannableStringBuilder style=new SpannableStringBuilder(show);
            style.setSpan(new ForegroundColorSpan(Color.BLUE), show.length() - 7, show.length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE); //设置指定位置文
            messageContent.setText(style);
            messageContent.setOnClickListener(new OnClickListener() {
                @Override
