...
import com.fhzc.app.android.helper.CircleBitmapDisplayer;
...
public class ImageLoaderUtil {
    ...
    public static void initImageLoaderConfig(Context context) {
        try {
            File cacheDir = StorageUtils.getOwnCacheDirectory(context, CacheUtil.getSavePath(Constant.FILE_SAVE_TYPE_IMAGE));
...
//                    .denyCacheImageMultipleSizesInMemory()
    ...
    public static DisplayImageOptions getAvatarOptions(int corner, int defaultRes){
        try {
            if (defaultRes <= 0) {
                defaultRes = R.drawable.im_default_user_portrait_corner;
            }
